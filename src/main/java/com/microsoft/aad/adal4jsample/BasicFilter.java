...
public class BasicFilter implements Filter {
            ...
            try {
                ...
                if (!AuthHelper.isAuthenticated(httpRequest)) {
                    if (AuthHelper.containsAuthenticationData(httpRequest)) {
                        logger.info(String.format("not authenticated but contains authentication - curUri=%s, fullUrl=%s", currentUri, fullUrl));
                        processAuthenticationData(httpRequest, currentUri, fullUrl);
                    } else {
                        // not authenticated
                        logger.info("http request contains no authenticated data");
    ...
    private void updateAuthDataUsingRefreshToken(HttpServletRequest httpRequest) throws Throwable {
        ...
        logger.info(String.format("get AuthenticationResult by refresh done - authResult=%s", authenticationResultToJson(authData).toString(2)));
    ...
    private void processAuthenticationData(HttpServletRequest httpRequest, String currentUri, String fullUrl)
        ...
        logger.info(String.format("start parse AuthenticationResponse - fullUri=%s, params=%s", fullUrl, params));
    ...
    private void validateNonce(StateData stateData, String nonce) throws Exception {
        logger.info(String.format("try validate nonce - nonce=%s, nonceInStateData=%s", nonce, stateData.getNonce()));
    ...
    private void sendAuthRedirect(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        ...
        String currentUri = httpRequest.getRequestURL().toString();
        String redirectedUrl = getRedirectUrl(currentUri, state, nonce);
        logger.info(String.format("redirect for authentication - curUrl=%s, redirectTo=%s", currentUri, redirectedUrl));
    ...
    private AuthenticationResult getAccessTokenFromRefreshToken(
        ...
        logger.info(String.format("get token from refreshToken - refreshToken=%s", refreshToken));
    ...
    private JSONObject authenticationResultToJson(AuthenticationResult toJson) throws JSONException {
        return new JSONObject()
                .put("accessTokenType", toJson.getAccessToken())
                .put("expiresOn", toJson.getExpiresOnDate())
                .put("idToken", toJson.getIdToken())
                .put("userInto", toJson.getUserInfo())
                .put("accessToken", toJson.getAccessToken())
                .put("refreshToken", toJson.getRefreshToken());
    }
